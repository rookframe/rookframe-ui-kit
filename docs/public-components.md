...
| `action_link.tscn` / `Button` | `title = "Action"`; `action_label = "›"`; `title_lane_width = 150.0`; `title_font_size = 15`; native `disabled = false` | Native `pressed` | Complete quiet navigation target with a label lane, intentional gap, continuous trailing rule, and action marker. The rule never touches the label and reaches the marker; callers own policy and connect `pressed`. |
